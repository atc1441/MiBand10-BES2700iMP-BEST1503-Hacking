...
{
    /* BES2700 port: compute 2^32/|v| directly. The Cortex-M33 has a hardware
     * divider, so GBADoom's 256KB reciprocalTable[65537] lookup (m_recip.c) is
     * not needed - dropping it frees ~256KB of flash. The result matches the
     * old table (table[i] = 2^32/i, clamped to 32 bits). */
    int neg = v < 0;
    unsigned int val = neg ? (unsigned int)(-v) : (unsigned int)v;
    if (val == 0)
        return 0;                                  /* matches old reciprocalTable[0] */
    unsigned long long r = 0x100000000ULL / val;   /* 2^32 / val */
    if (r > 0xFFFFFFFFULL)
        r = 0xFFFFFFFFULL;                         /* clamp like the 32-bit table */
    fixed_t result = (fixed_t)(unsigned int)r;
