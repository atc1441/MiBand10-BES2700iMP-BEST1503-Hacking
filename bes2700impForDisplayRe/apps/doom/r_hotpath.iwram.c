...
static void R_DrawColumn(const draw_column_vars_t *dcvars)
{
    ...
    unsigned short *dest = drawvars.byte_topleft + dcvars->yl * SCREENWIDTH + dcvars->x;
    ...
    while (l--)
    {
        R_DrawColumnPixel(dest, source, colormap, frac);
        dest += SCREENWIDTH;
        frac += fracstep;
        R_DrawColumnPixel(dest, source, colormap, frac);
        dest += SCREENWIDTH;
...
static void R_DrawFuzzColumn(const draw_column_vars_t *dcvars)
{
    ...
    unsigned short *dest = drawvars.byte_topleft + dc_yl * SCREENWIDTH + dcvars->x;

    unsigned int fuzzpos = _g->fuzzpos;

    do
    {
        R_DrawColumnPixel((pixel *)dest, &dest[fuzzoffset[fuzzpos]], colormap, 0);
        dest += SCREENWIDTH;
        fuzzpos++;
...
static void R_DrawSpan(unsigned int y, unsigned int x1, unsigned int x2, const draw_span_vars_t *dsvars)
{
    ...
    unsigned short *dest = drawvars.byte_topleft + y * SCREENWIDTH + x1;
    ...
    while (l--)
    {
        R_DrawSpanPixel(dest, source, colormap, position);
        dest++;
        position += step;
        R_DrawSpanPixel(dest, source, colormap, position);
        dest++;
...
static fixed_t R_ScaleFromGlobalAngle(angle_t visangle)
{
    ...
    return den > num >> 16 ? (num = FixedDiv(num, den)) > 64 * FRACUNIT ? 64 * FRACUNIT : num < 256 ? 256
                                                                                                    : num
                           : 64 * FRACUNIT;
}
...
static void R_RecalcLineFlags(void)
{
    ...
    if (!(linedef->flags & ML_TWOSIDED) || backsector->ceilingheight <= frontsector->floorheight || backsector->floorheight >= frontsector->ceilingheight || (
                                                                                                                                                                 ...
                                                                                                                                                                 && (backsector->ceilingheight >= frontsector->ceilingheight || side->toptexture)

                                                                                                                                                                 && (backsector->floorheight <= frontsector->floorheight || side->bottomtexture)
    ...
    else
    {
        ...
        if (backsector->ceilingheight != frontsector->ceilingheight || backsector->floorheight != frontsector->floorheight || side->midtexture || backsector->ceilingpic != frontsector->ceilingpic || backsector->floorpic != frontsector->floorpic || backsector->lightlevel != frontsector->lightlevel)
        {
            linedata->r_flags = (linedata->r_flags & ML_MAPPED);
            return;
        }
        else
...
static boolean R_CheckBBox(const short *bspcoord)
{
...

    {
        ...
        boxpos = (viewx <= ((fixed_t)bspcoord[BOXLEFT] << FRACBITS) ? 0 : viewx < ((fixed_t)bspcoord[BOXRIGHT] << FRACBITS) ? 1
                                                                                                                            : 2) +
                 (viewy >= ((fixed_t)bspcoord[BOXTOP] << FRACBITS) ? 0 : viewy > ((fixed_t)bspcoord[BOXBOTTOM] << FRACBITS) ? 4
                                                                                                                            : 8);
...
{
    fixed_t left, right;
    return !node->dx   ? x == node->x ? 2 : x <= node->x ? node->dy > 0
                                                         : node->dy < 0
           : !node->dy ? (y) == node->y ? 2 : y <= node->y ? node->dx < 0
                                                           : node->dx > 0
...
/* BES2700 port: the 35Hz DOOM tick is derived from the firmware's free-running
 * system timer below. The original SysTick_Handler is removed because RTX5 owns
 * the SysTick vector (it would be a duplicate-symbol clash at link time). */
#include "hal_timer.h"
static int I_GetTime_e32(void)
{
    return (int)((uint32_t)TICKS_TO_MS(hal_sys_timer_get()) * TICRATE / 1000u);
}
