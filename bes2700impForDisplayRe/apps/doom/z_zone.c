...
/* BES2700 port: the zone's underlying block allocator uses the dedicated doom
 * arena (apps/doom/bes_glue.c) instead of newlib malloc, whose _sbrk heap is
 * only 4KB. All doom memory funnels through Z_Malloc -> these two calls. */
...
void *Z_Malloc_verbose(size_t size, int tag, void **user, const char *function, int line)
{
  ...
  while (!(block = doom_malloc(size + HEADER_SIZE))) {
  ...
  /* BES: per-alloc trace floods the 921600 UART and starves the frame loop. */
  /* printf("Alloc %s %d: %d (%d)\n", function, line, block->size, running_count); */
