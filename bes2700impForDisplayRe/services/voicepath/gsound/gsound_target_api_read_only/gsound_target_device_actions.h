 ...
 * state changes.  As a guideline, states should not be defined that normally
 * change more frequently than every 15 seconds. If a state does occasionally
 * update more often than that, that is fine. However, it might result in a
 * query using an older value of this state. It is most important that the
 * device reports the newest/lastest reading, even if it comes in late by
 * 15 seconds or was just updated very recently. A reasonable update interval is
 * state dependent. When GSound is done with the data from a
 * gsound_target_device_actions_state_update, GSound will call
 * GSoundTargetDeviceActionsStateUpdateConsumed.  This call will be made for all
 * gsound_target_device_actions_state_update callbacks, both the initial call
 * and all subsequent update calls.
 *
...
typedef struct {
   ...
   * 15 seconds. See detailed explanation in the high level flow at the top of
   * this API header.
   *
