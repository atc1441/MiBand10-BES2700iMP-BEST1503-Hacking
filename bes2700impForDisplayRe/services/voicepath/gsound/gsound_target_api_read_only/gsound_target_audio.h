...
typedef struct {
   ...
   * data to GSound.
   *
   * Note: For products that use internal hotword, this function will handle
   * clearing the hotword buffer and restarting detection after an HFP session.
   * For products that use external hotword, Target is responsible for clearing
   * the hotword buffer and restarting detection (if the audio stream to the
   * external hotword detector was interrupted and GSound did not stop detection
   * during the HFP session).
   *
...
/**
 * Stop sending GSound audio and close side-tone. The Target is responsible for
 * deciding whether or not to close the mic in case there are other consumers of
 * mic audio data (for example: external hotword detection, noise cancellation).
 * Whether or not the mic is closed, GSound should not receive any audio after
 * this call.
 *
 ...
 * GSoundTargetAudioInClose or GSoundTargetAudioUndoPrepareForStreaming.
 *
...
/**
 * The target should undo any preparation done in
 * GSoundTargetAudioPrepareForStreaming
 *
 * Note, if a Target does not require undoing any configuration, simply return
 * GSOUND_STATUS_OK.
 *
 */
