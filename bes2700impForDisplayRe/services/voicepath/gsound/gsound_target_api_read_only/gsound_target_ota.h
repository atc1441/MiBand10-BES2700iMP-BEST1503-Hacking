 ...
 * OTA Description:
 *
 * We support two types of OTA.
 * 1) Full-Firmware OTA: The transfer and update of the entire firmware image to
 *    the device.
 * 2) Model OTA: The transfer of a Hotword model data-only file.
 *
 ...
 *
 * Notes on TWS (multi-part) OTA:
 * - To simplify the TWS OTA process, we require that both the Master and Slave
 *   earbuds are present and connected during the entire duration of a
 *   Full-Firmware OTA session. A Hotword Model OTA session can begin and
 *   proceed to completion with a missing Slave.
 *
 * - The Target should reject or terminate a TWS OTA session under any of the
 *   following conditions:
 *      1) The Slave is missing when OTA begins. Full-Firmware only.
 *      2) The Slave goes missing after OTA begins. Full-Firmware only.
 *      3) A role switch occurs after OTA begins. Full-firmware and Model OTA.
 *
 * - Example: If placing the Slave earbud in the case causes it to disconnect
 *   from Master, an in-progress Full-Firmware OTA session is expected to be
 *   terminated by the Target.
 *
 * - Example: If placing the Slave earbud in the case causes it to disconnect
 *   from Master, an in-progress Model OTA session can proceed as normal.
 *
 * - Example: If placing the Master earbud in the case triggers a role change,
 *   an in-progress Full-Firmware or Model OTA session is expected to be
 *   terminated by the Target.

 * - Example: If only the Master earbud is present, an attempt to begin a
 *   Full-Firmware OTA session should be rejected by the Target.
 *
 * - Example: If only the Master earbud is present, an attempt to begin a Model
 *   OTA session can proceed as normal.

 * - Example: If both earbuds are in the case but connected to the phone, an OTA
 *   session can proceed as normal
 *
 * - Example: If either the Master or Slave earbud is place in the case after
 *   OTA begins, but remains normally connected without a role change, an
 *   in-progress OTA session can proceed as normal.
 *
 */
...
typedef struct {
  ...
  /**
   * Set this value to True only if the Target is requesting a rewind.
   */
   ...
   * more details. This value is ignored if rewind_requested is false.
   */
