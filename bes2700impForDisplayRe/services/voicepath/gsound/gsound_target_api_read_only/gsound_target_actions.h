...
typedef enum {
  ...
  /**
   * Event to be sent when starting an auto-endpointing query. Assistant on the
   * phone makes the final decision to start the query, so mic input audio will
   * not start streaming immediately.
   */
  GSOUND_TARGET_ACTION_GA_START_ENDPOINTING_QUERY = (1 << 16),

  /**
   * Event to be sent when manually ending an auto-endpointing query. Assistant
   * on the phone makes the final decision to end the query, so mic input audio
   * will continue streaming for a short time after this action is received. If
   * this Action is not sent, the Assistant will eventually close the mic
   * automatically.
   */
  GSOUND_TARGET_ACTION_GA_END_ENDPOINTING_QUERY = (1 << 17),

  /**
   * Event to be sent when a Play Action request has occurred. This should be
   * used when events that require an explicit play command occur (e.g. when
   * two buds in a TWS headset are put on) instead of a play/pause toggle.
   */
  GSOUND_TARGET_ACTION_PLAY = (1 << 18),

  /**
   * Event to be sent when a Pause Action request has occurred. This should be
   * used when events that require an explicit pause command occur (e.g. when
   * a bud in a TWS headset is removed) instead of a play/pause toggle.
   */
  GSOUND_TARGET_ACTION_PAUSE = (1 << 19),

  /**
   * DO NOT USE
   */
  GSOUND_TARGET_ACTION_MAX = (1 << 20),
} GSoundActionEvents;
...
/**
 * The following lists the different ways Assistant may be available as a gesture
 * on the device.
 */
typedef enum {
  GSOUND_ASSISTANT_GESTURE_AVAILABILITY_UNSET = 0,

  // No Assistant gesture is available on any of the devices
  GSOUND_ASSISTANT_GESTURE_UNAVAILABLE,

  // An Assistant gesture is available only on the left part of a multi-part device
  GSOUND_ASSISTANT_GESTURE_AVAILABLE_LEFT,

  // An Assistant gesture is available only on the right part of a multi-part
  // device
  GSOUND_ASSISTANT_GESTURE_AVAILABLE_RIGHT,

  // An Assistant gesture is available on both parts of a multi-part device
  GSOUND_ASSISTANT_GESTURE_AVAILABLE_BOTH,

  // An Assistant gesture is available on a single-part device
  GSOUND_ASSISTANT_GESTURE_AVAILABLE_SINGULAR
} GSoundAssistantGestureAvailability;
...
typedef struct {
  ...
  /**
   * Callback to execute when the Assistant gesture changes.
   *
   * This should only be executed when the Assistant gesture mapping changes on
   * a device which supports gesture customizations. For devices whose gesture
   * space cannot be customized, there is no need to call this function.
   *
   * Note: For TWS devices, this should be called when either device’s gesture
   * mapping changes, even if that device is disconnected.
   *
   * param[in]: availability The new availability of the Assistant gesture.
   */
  GSoundStatus (*gsound_action_on_assistant_gesture_change)(
 ...
 * Target.
 *
 * In a multi-point configuration, the target may need to perform a resume of
 * the secondary on a rejected media action depending on current state. See
 * GSoundTargetBtResumeSecondary.
 *
...
/**
 * Returns the current Assistant gesture status
 *
 * See the definition of GSoundActionAssistantGestureStatus for details.
 *
 * Note: For TWS devices, this should be reflect the status of both devices,
 * even if one of the devices is disconnected.
 *
 * param[out]: availability The current availability of the Assistant gesture
 *
 * return: GSOUND_STATUS_OK if the availability output argument was set
 *         GSOUND_STATUS_NOT_SUPPORTED if getting the availability of the
 *                                     Assistant gesture is not supported
 *         GSOUND_STATUS_ERROR on any other error
 */
