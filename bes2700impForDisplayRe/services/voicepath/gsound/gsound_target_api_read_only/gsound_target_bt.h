...
/**
 * Types of active audio sources. A source is considered active if it is streaming
 * audio data over A2DP or HFP. Non-connected devices are always considered
 * inactive.
 */
typedef enum {
  GSOUND_ACTIVE_AUDIO_SOURCE_UNSET = 0,

  // Detection of the active audio source is not supported
  GSOUND_ACTIVE_AUDIO_SOURCE_UNSUPPORTED,

  // No device is actively streaming audio
  GSOUND_ACTIVE_AUDIO_SOURCE_NONE,

  // The primary BT connection is actively streaming audio
  GSOUND_ACTIVE_AUDIO_SOURCE_PRIMARY,

  // The secondary BT connection is actively streaming audio
  GSOUND_ACTIVE_AUDIO_SOURCE_SECONDARY
} GSoundActiveAudioSource;
...
typedef struct {
  ...
  /**
   * Callback which should be executed by Target when the current HFP state
   * changes. This could be when a phone call or VOIP call is started or ended.
   *
   * This function is non-blocking and thread safe. It can be called from any
   * thread/context including an ISR.
   *
   * Note: Both this function and audio_in_stream_interrupted from the audio
   * Target API should be called when HFP is deactivated.
   *
   * param[in]: is_hfp_active  Whether the new HFP state is active
   */
  GSoundStatus (*gsound_bt_on_hfp_state_change)(bool is_hfp_active);

  /**
   * Callback which should be executed by Target when the current active audio
   * source changes.
   *
   * This function is non-blocking and thread safe. It can be called from any
   * thread/context including an ISR.
   *
   * param[in]: audio_source  New active audio source
   */
  GSoundStatus (*gsound_bt_on_active_audio_source_change)(
      GSoundActiveAudioSource audio_source);
} GSoundBtInterface;
 ...
 *
 * In a multi-point configuration, when the target receives a rejected media
 * action (see GSoundTargetActionRejected), the target may use this information
 * to resume the secondary if appropriate based on the target's multi-point
 * state. Libgsound does not know the current state of the devices and,
 * therefore, does not know whether or not to resume.
 *
 * param[in]: gsound_addr Pointer to BD_ADDR of GSound Device
 */
...
/**
 * Returns the current active audio source. See definition of
 * GSoundActiveAudioSource for definitions of what to return.
 *
 * param[out]: audio_source_out  Currently active audio source
 *
 * returns:  GSOUND_STATUS_OK     If the audio source was set correctly. If
 *                                detection of the active audio source is not
 *                                supported, set to
 *                                GSOUND_ACTIVE_AUDIO_SOURCE_NOT_SUPPORTED.
 *           GSOUND_STATUS_ERROR  For all other errors
 */
