...
# NOTE: re-quoting the full command via 'cmd_$@ := $(make-cmd)' breaks under
# mingw32-make's /usr/bin/sh when a command contains a quote/paren.  We don't
# need command-change detection for a clean build, so write a constant marker
# instead (keeps the .cmd format valid; just forces a rebuild every time).
depfile-new = printf '\ncmd_$@ := 1\n' > $(depfile)
depfile-add = printf '\ncmd_$@ := 1\n' >> $(depfile)
endif
