...
# best1306 SDK: some $(CHIP)/ subdirs (e.g. drivers/bt/best1503) have no Makefile.
# Tolerate that (the empty built-in.a is then skipped via $(wildcard) in archive-cmd).
-include $(kbuild-file)
...
# Response-file helper: on make >= 4.0 write the (possibly huge) flag list to
# $@.rsp via $(file) and pass gcc "@$@.rsp", so the recipe command line stays short.
# mingw32-make/Windows truncates command lines past ~8191 ("sh: line 2: syntax
# error"); make 3.82 has no $(file), so it falls back to inline flags.
ifeq ($(filter 4.% 5.%,$(MAKE_VERSION)),)
ccflags_rsp = $(1)
else
ccflags_rsp = $(file >$@.rsp,$(1))@$@.rsp
endif

cmd_cc_o_c       = $(CC) $(call ccflags_rsp,$(c_flags)) -c -o $@ $<
...
# Use if_changed_dep (single-line recipe) instead of if_changed_rule: the latter
# expands the multi-line `define rule_cc_o_c`, which mingw32-make mangles into a
# 2-line shell command ("syntax error: unexpected end of file") on Windows.
$(obj)/%.o: $(src)/%.c FORCE
...
cmd_cc_o_c++       = $(C++) $(call ccflags_rsp,$(c++_flags)) -c -o $@ $<
...
cmd_as_o_S       = $(CC) $(call ccflags_rsp,$(a_flags)) -c -o $@ $<
